...
class BufDesc {
	...
	// Allow BufMgr access to the private members of this class.
	friend class BufMgr;
 ...
 private:
  ...
  void Set(File* filePtr, PageId pageNum)
	...
	/**
	 * Print out information about this BufDesc object
	 */
  void Print()
	{
		if(file != NULL)
...
class BufMgr 
{
 private:
  ...
  void allocBuf(FrameId & frame);
	...
	/**
	 * Reads a page that's already in the buffer pool. Helper function for
	 * readPage().
	 * @param file The file containing the page to read.
	 * @param pageNo The page number of the page to read.
	 * @param page page reference, the desired page will be returned via this
	 * variable.
	 * @param frameToReadFrom ID of the frame containing the page to be read.
	 */
	void readPageInBufferPool(File* file, const PageId pageNo, Page*& page, FrameId frameToReadFrom);

	/**
	 * Reads a page that's not already in the buffer pool. Helper function for
	 * readPage().
	 * @param file The file containing the page to read.
	 * @param pageNo The page number of the page to read.
	 * @param page page reference, the desired page will be returned via this
	 * variable.
	 */
	void readPageNotInBufferPool(File* file, const PageId pageNo, Page*& page);
 ...
 public:
  ...
  ~BufMgr();
	 ...
	 * @throws			  BufferExceededException If desired page is not in the buffer pool and all frames are already allocated
	 * @throws			  InvalidPageException If the desired page does not exist.
	 */
  ...
  void unPinPage(File* file, const PageId PageNo, const bool dirty);
	 ...
	 * @throws				BufferExceededException If all frames in the buffer pool are already allocated.
	 */
