...
void test7()
{
	// See if the destructor functions properly on an empty buffer pool.
	// Mainly for use with valgrind to ensure everything is torn down correctly.
	BufMgr* test7BufMgr = new BufMgr(50);
	delete test7BufMgr;
...
void test8()
{
	// Read page not yet in the buffer pool.
	BufMgr* test8BufMgr = new BufMgr(10);

	Page* pageFromTest2;
	test8BufMgr->readPage(file2ptr, pageno2, pageFromTest2);
	// These page numbers should be equal
	if (pageno2 != pageFromTest2->page_number()) {
		PRINT_ERROR("ERROR :: Page numbers didn't match.");
	}

	// Read page already in buffer pool
	test8BufMgr->readPage(file2ptr, pageno2, pageFromTest2);
	// These page numbers should be equal
	if (pageno2 != pageFromTest2->page_number()) {
		PRINT_ERROR("ERROR :: Pages didn't match.");
	}
...
void test9()
{
	// stress test with sequential flooding

	// Buffer manager has 1 fewer than num pages that will be read
	BufMgr* test9BufMgr = new BufMgr(99);
	Page* test9Page;

	// Go through several cycles of reading in all pages
	for (int j = 0; j < 10; ++j) {
		// Read in all pages
		for (i = 0; i < num; i++) {
			test9BufMgr->readPage(file1ptr, pid[i], test9Page);
			test9BufMgr->unPinPage(file1ptr, pid[i], false);
		}
	}
...
void test10()
{
	// tests unPinPage() on page not in buffer pool (nothing should happen)
	BufMgr* test10BufMgr = new BufMgr(5);
	test10BufMgr->unPinPage(file1ptr, pid[0], false);
